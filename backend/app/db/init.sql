...
IF NOT EXISTS (SELECT 1 FROM settings_group)
INSERT INTO settings_group (group_name, description)
VALUES ('PRODUCTIVITY', 'Productivity thresholds and policy controls'),
       ('SLA', 'SLA thresholds and reminders'),
       ('HELP_RELEASE', 'Help refresh and release-note publication controls');

IF NOT EXISTS (SELECT 1 FROM settings_registry)
INSERT INTO settings_registry (settings_group_id, setting_key, setting_label, setting_value, value_type, module_name, scope_type, is_editable, is_active)
VALUES (1, 'idle_threshold_minutes', 'Idle Threshold (Minutes)', '5', 'INT', 'PRODUCTIVITY', 'GLOBAL', 1, 1),
       (2, 'sla_breach_warning_hours', 'SLA Breach Warning (Hours)', '4', 'INT', 'SLA_ENGINE', 'GLOBAL', 1, 1),
       (3, 'help_refresh_mode', 'Help Refresh Mode', 'AUTO_DRAFT', 'STRING', 'HELP', 'GLOBAL', 1, 1);
