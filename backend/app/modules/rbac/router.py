...
@router.post("/permissions/tree/seed")
def seed_permission_tree(db: Session = Depends(get_db)):
    return RbacService(db).seed_permission_tree()


@router.get("/permissions/tree")
def permission_tree(db: Session = Depends(get_db)):
    return RbacService(db).permission_tree()


@router.post("/roles/seed-templates")
def seed_role_templates(db: Session = Depends(get_db)):
    return RbacService(db).seed_role_templates()
...
@router.put("/users/{user_id}/access-profile")
def upsert_access_profile(user_id: int, payload: UserAccessProfileRequest, db: Session = Depends(get_db)):
    return RbacService(db).upsert_user_access_profile(user_id, payload)


@router.put("/users/{user_id}/scopes")
def set_user_scope(user_id: int, payload: UserScopeMapRequest, db: Session = Depends(get_db)):
    return RbacService(db).set_user_scope(user_id, payload)


@router.get("/users/{user_id}/scopes")
def get_user_scope(user_id: int, db: Session = Depends(get_db)):
    return RbacService(db).get_user_scope(user_id)
...
@router.post("/access/effective")
def effective_access(payload: EffectiveAccessRequest, db: Session = Depends(get_db)):
    return RbacService(db).effective_permission(payload)
...
@router.get("/reports")
def rbac_reports(db: Session = Depends(get_db)):
    return RbacService(db).reports()
