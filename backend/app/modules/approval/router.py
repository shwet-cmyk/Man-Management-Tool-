...
@router.get("/list")
def approval_list(
    approval_type: ApprovalModule | None = None,
    module: ApprovalModule | None = None,
    requested_by: int | None = None,
    current_approver: int | None = None,
    status: ApprovalState | None = None,
    priority: str | None = None,
):
    rows = list(APPROVALS.values())
    filter_module = module or approval_type
    if filter_module:
        rows = [r for r in rows if r["module"] == filter_module]
    if requested_by is not None:
        rows = [r for r in rows if r["creator_user_id"] == requested_by]
    if current_approver is not None:
        rows = [r for r in rows if r.get("level1_approver_user_id") == current_approver]
    if status:
        rows = [r for r in rows if r["state"] == status]
    for r in rows:
        r["priority"] = priority or r.get("priority", "MEDIUM")
        r["sla_due"] = (r["created_at"] + timedelta(hours=APPROVAL_CONFIGS[r["module"]]["escalation_hours"])).isoformat()
...
def approval_detail_contract(approval_id: int):
...
def unified_action(approval_id: int, payload: dict):
    action = (payload.get("action") or "").upper()
    if action not in {"APPROVE", "REJECT", "SEND_BACK", "REASSIGN", "ESCALATE"}:
        raise HTTPException(status_code=422, detail="Invalid action")
    if action == "REASSIGN":
        return reassign_approval(approval_id, payload.get("reassignTo"))
    if action == "ESCALATE":
        return escalate_approval(approval_id)
    return approval_action(
        approval_id,
        ApprovalActionRequest(
            actor_user_id=payload.get("actorUserId", 0),
            actor_role=payload.get("actorRole", "Admin"),
            action=action,
...
def approve_approval(approval_id: int, actor_user_id: int = 0, actor_role: str = "Admin", remarks: str | None = None):
    return approval_action(approval_id, ApprovalActionRequest(actor_user_id=actor_user_id, actor_role=actor_role, action="APPROVE", remarks=remarks))


@router.post("/reject/{approval_id}")
def reject_approval(approval_id: int, remarks: str, actor_user_id: int = 0, actor_role: str = "Admin"):
    return approval_action(approval_id, ApprovalActionRequest(actor_user_id=actor_user_id, actor_role=actor_role, action="REJECT", remarks=remarks))


@router.post("/send-back/{approval_id}")
def send_back_approval(approval_id: int, remarks: str, actor_user_id: int = 0, actor_role: str = "Admin"):
    return approval_action(approval_id, ApprovalActionRequest(actor_user_id=actor_user_id, actor_role=actor_role, action="SEND_BACK", remarks=remarks))


def reassign_approval(approval_id: int, target_user_id: int | None):
    row = APPROVALS.get(approval_id)
    if not row:
        raise HTTPException(status_code=404, detail="Approval record not found")
    if not target_user_id or target_user_id not in _USERS:
        raise HTTPException(status_code=422, detail="Reassign target must be active and eligible")
    row["level1_approver_user_id"] = target_user_id
    row["updated_at"] = datetime.utcnow()
    _log("APPROVAL_REASSIGNED", {"approval_id": approval_id, "target_user_id": target_user_id})
    return row


@router.post("/reassign/{approval_id}")
def reassign_approval_contract(approval_id: int, target_user_id: int):
    return reassign_approval(approval_id, target_user_id)


@router.post("/escalate/{approval_id}")
def escalate_approval(approval_id: int):
    row = APPROVALS.get(approval_id)
    if not row:
        raise HTTPException(status_code=404, detail="Approval record not found")
    row["escalated"] = True
    row["updated_at"] = datetime.utcnow()
    _log("APPROVAL_ESCALATED_MANUAL", {"approval_id": approval_id})
    return row


@router.post("/bulk-approve")
def bulk_approve(approval_ids: list[int], actor_user_id: int = 0, actor_role: str = "Admin"):
    rows = [APPROVALS.get(i) for i in approval_ids]
    if any(r is None for r in rows):
        raise HTTPException(status_code=404, detail="One or more approvals not found")
    modules = {r["module"] for r in rows if r}
    if len(modules) > 1:
        raise HTTPException(status_code=422, detail="Bulk actions allowed only on same approval type where configured")
    acted = []
    for approval_id in approval_ids:
        row = APPROVALS[approval_id]
        if row["state"] in {ApprovalState.approved, ApprovalState.rejected, ApprovalState.sent_back}:
            continue
        approval_action(approval_id, ApprovalActionRequest(actor_user_id=actor_user_id, actor_role=actor_role, action="APPROVE", remarks="Bulk approve"))
        acted.append(approval_id)
    _log("APPROVAL_BULK_APPROVE", {"approval_ids": acted})
    return {"processed": len(acted), "approval_ids": acted}
