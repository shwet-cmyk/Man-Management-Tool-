...
from app.productivity.api._deps import audit_service, policy_service
...
async def upsert_policy(payload: ProductivityPolicyUpsertRequest):
    ...
    await audit_service.log(user_id=None, action_type='POLICY_UPSERT', target_entity='productivity_policy', target_id=row['id'], remarks=row['policy_name'])
    return ProductivityPolicyResponse(
